...
- [ ] **Step 1: Add unit tests for new multi-dial exporter options**
  ...
  ```typescript
  	it('filters by selectedGroupIds when provided', () => {
  		...
  		const result = exportLaserSvg(content, SAMPLE_CONFIG, layers, { selectedGroupIds: ['g1'] }, groups);
  	...
  	it('applies independent numbering scheme when selected', () => {
  		...
  		const result = exportLaserSvg(content, SAMPLE_CONFIG, layers, { numberingScheme: 'independent' }, groups);
  	...
  	it('formats titles correctly based on titleMode options', () => {
  		...
  		const resultName = exportLaserSvg(content, SAMPLE_CONFIG, layers, {
  			dialTitle: 'MyProject',
  			titleMode: 'name'
  		}, groups);
  		...
  		const resultNumbered = exportLaserSvg(content, SAMPLE_CONFIG, layers, {
  			dialTitle: 'MyProject',
  			titleMode: 'numbered'
  		}, groups);
  		...
  		const resultBoth = exportLaserSvg(content, SAMPLE_CONFIG, layers, {
  			dialTitle: 'MyProject',
  			titleMode: 'both'
  		}, groups);
...
- [ ] **Step 2: Add UI controls in HTML template**
  				...
  				<div class="mt-4 flex flex-col gap-4">
  					...
  					{#if doodledialStore.groups.length > 1}
  						<div>
  							...
  							<div class="mt-2 flex flex-col gap-2 rounded-lg border border-gray-200 p-3 max-h-36 overflow-y-auto">
  ...
  ```svelte
  				<button
  					...
  					disabled={selectedFormat === 'laser-svg' && doodledialStore.groups.length > 1 && selectedGroupIds.length === 0}
